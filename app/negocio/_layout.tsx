/**
 * app/negocio/_layout.tsx
 * ─────────────────────────────────────────────────────────────
 * Layout protegido del Panel Anfitrión de CaseritaExpress.
 *
 * Guard: verifica autenticación y rol 'anfitrion' en profiles.
 * Mismo patrón exacto que app/admin/_layout.tsx y app/repartidor/_layout.tsx.
 * Tema: verde (#16A34A) para diferenciarlo de admin y repartidor.
 * 4 tabs: Pedidos | Menú | Fotos | Estadísticas
 * ─────────────────────────────────────────────────────────────
 */
...
const C = {
  ...
  tabActive:     '#16A34A',
  tabInactive:   '#9CA3AF',
  sidebar:       '#052E16',   // verde muy oscuro
  sidebarText:   '#FFFFFF',
  sidebarSub:    '#86EFAC',
...
export default function NegocioLayout() {
  ...
  // ── Guard: mismo patrón que admin y repartidor ────────────
  const checkAccess = useCallback(async () => {
    try {
      ...
      if (profileError || profile?.rol !== 'anfitrion') {
        router.replace('/')
        return
      }
      ...
      setNombre(profile.nombre ?? user.email ?? 'Anfitrión')
      setNegocioName(negocio?.nombre ?? 'Mi negocio')
      setAutorizado(true)
    } catch {
      router.replace('/')
    } finally {
      ...
      <View style={styles.webContainer}>
        <View style={styles.sidebar}>
          <View style={styles.sidebarHeader}>
            ...
            <Text style={styles.sidebarTitle} numberOfLines={1}>{negocioName}</Text>
            <Text style={styles.sidebarSubtitle}>Panel Anfitrión</Text>
          </View>
          ...
          <View style={styles.sidebarFooter}>
            <Text style={styles.sidebarName} numberOfLines={1}>👤 {nombre}</Text>
  ...
  return (
    <SafeAreaView style={styles.mobileContainer}>
      <View style={styles.mobileHeader}>
        ...
        <Text style={styles.mobileHeaderTitle} numberOfLines={1}>{negocioName}</Text>
...
const styles = StyleSheet.create({
  ...
  sidebar:      { width: 230, backgroundColor: C.sidebar, flexDirection: 'column' },
  sidebarHeader: {
    paddingHorizontal: 20, paddingTop: 32, paddingBottom: 24,
    borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.1)',
  },
  sidebarLogo:     { fontSize: 28, marginBottom: 6 },
  sidebarTitle:    { color: C.sidebarText, fontSize: 15, fontWeight: '700' },
  sidebarSubtitle: { color: C.sidebarSub, fontSize: 11, marginTop: 2 },
  ...
  mobileHeaderTitle: { color: C.sidebarText, fontSize: 14, fontWeight: '700', flex: 1 },
  mobileHeaderName:  { color: C.sidebarSub, fontSize: 11, maxWidth: 80 },
  mobileSignOut:     { color: C.sidebarSub, fontSize: 13, fontWeight: '600' },
